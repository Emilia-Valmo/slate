import { Block, Change, Inline, Mark, Plugin, Range, Schema, Stack, Value } from '@gitbook/slate';
import {
  IS_ANDROID,
  IS_FIREFOX,
  IS_IOS,
...
function Editor(props: EditorProps): React.Node {
  const { value, readOnly, role, tabIndex, style, plugins: propPlugins, schema: propSchema } = props
  ...
  const { stack, schema } = React.useMemo(() => {
    ...
    return { stack: Stack.create({ plugins }), schema: Schema.create({ plugins }) }
  ...
  const onEvent = (handler: string, event: React.SyntheticEvent<HTMLElement>) => {
 ...
 */
function updateSelection(element: HTMLElement, readOnly: boolean, value: Value, isUpdatingSelectionRef: React.Ref<boolean>): void {
 ...
 */
function isInEditor(element: HTMLElement, target: HTMLElement, mustBeEditable: boolean = true): boolean {
  let el: HTMLElement
