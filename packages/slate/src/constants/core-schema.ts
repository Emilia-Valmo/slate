 ...
 */
function onlyBlocksInDocument(node) {
 ...
 */
function consistentBlockChildren(node) {
 ...
 */
function inlinesContainInlinesOrText(node) {
 ...
 */
function inlinesAreNotEmpty(node) {
...
/*
 * Ensure that inline nodes are surrounded by text nodes, by adding
 * extra blank text nodes if necessary.
 */
function inlinesAreBetweenTexts(node) {
    if (node.object !== 'block' && node.object !== 'inline') {
        return;
    }
 ...
 */
function removeExtraEmptyTexts(node) {
    if (node.object !== 'block' && node.object !== 'inline') {
        return;
    }
    const { nodes } = node;
...
const CORE_SCHEMA = Schema.create({
    // Complexities are function of:
    // r: Number of root nodes
    // n: Total number of nodes
    // c: Average number of direct children
    validations: [
        // O(r)
        // onlyBlocksInDocument,
        // O(n*c)
        // consistentBlockChildren,
        // O(n*c)
        // inlinesContainInlinesOrText,
        // O(n)
        atLeastOneChild,
        // O(n*c)
        inlinesAreNotEmpty,
