...
class Node {
    ...
    public static fromJS = Node.fromJSON;
    /*
    ...
    public filterDescendants(iterator) {
        const matches = [];
    ...
    public findDescendant(iterator) {
        let found = null;
    ...
    public forEachDescendant(iterator) {
        let ret;
    ...
    public getBlocksAtRangeAsArray(range) {
    ...
    public getCharactersAtRange(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return List();
        }
    ...
    public getClosest(key, iterator) {
        key = assertKey(key);
    ...
    public getCommonAncestor(one, two) {
        one = assertKey(one);
        two = assertKey(two);
    ...
    public getDescendantAtPath(path) {
        let descendant = this;
    ...
    public getFragmentAtRange(range) {
        range = range.normalize(this);
        if (range.isUnset) {
    ...
    public getFurthest(key, iterator) {
        const ancestors = this.getAncestors(key);
    ...
    public getFurthestOnlyChildAncestor(key) {
        const ancestors = this.getAncestors(key);
    ...
    public getInlinesAtRange(range) {
    ...
    public getInlinesAtRangeAsArray(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return [];
        }
    ...
    public getInsertMarksAtRange(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return Set();
        }
    ...
    public getOrderedMarksAtRange(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return OrderedSet();
        }
    ...
    public getActiveMarksAtRange(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return Set();
        }
    ...
    public getMarksAtPosition(key, offset) {
    ...
    public getOrderedMarksByType(type) {
        const array = this.getMarksByTypeAsArray(type);
        return new OrderedSet(array);
    }
    ...
    public getTextsBetweenPositionsAsArray(startKey, endKey) {
        const startText = this.getDescendant(startKey);
    ...
    public getTextsAtRangeAsArray(range) {
        range = range.normalize(this);
        if (range.isUnset) {
            return [];
        }
    ...
    public hasInlines(key) {
        const node = this.assertNode(key);
        return !!(
            node.nodes &&
            node.nodes.find(n => Inline.isInline(n) || Text.isText(n))
        );
    }
    ...
    public mergeNode(withIndex, index) {
        let node = this;
    ...
    public mapDescendants(iterator) {
        let { nodes } = this;
    ...
    public splitNode(index, position) {
        let node = this;
