                ...
                if (text.__decorations) {
                    ...
                    text.__decorations
                        ...
                        .forEach(partial => {
                            ...
                            partialDecorations[partial._key] = partial.withKey(
                                text.key
                            );
                        });
                }
...
function createChildren(children, options = {}) {
    ...
    let node = Text.create({
        key,
        leaves: [{ text: '', marks: options.marks }]
    });
    ...
    function setNode(next) {
        ...
        if (__focus != null) {
            next.__focus = __focus;
        }
        if (__decorations != null) {
            next.__decorations = __decorations;
        }
        node = next;
    }
    ...
    children.forEach((child, index) => {
        ...
        if (Text.isText(child)) {
            ...
            child.getLeaves().forEach(leaf => {
                let { marks } = leaf;
                if (options.marks) {
                    marks = marks.union(options.marks);
                }
            ...
            if (__anchor != null) {
                node.__anchor = __anchor + length;
            }
            if (__focus != null) {
                node.__focus = __focus + length;
            }
        ...
        if (child === ANCHOR || child === CURSOR) {
            node.__anchor = length;
        }
        if (child === FOCUS || child === CURSOR) {
            node.__focus = length;
        }
